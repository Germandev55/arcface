...
def predict():  
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # print(frame_bytes)
            jpg_data = base64.b64decode(frame_bytes)
            #print(jpg_data, "jpg_data done")
            frame = cv2.imdecode(np.fromstring(jpg_data, dtype=np.uint8), cv2.IMREAD_UNCHANGED)
            ...
            cv2.imshow('face Capture'+str(args.port), frame)
            cv2.waitKey(1)
